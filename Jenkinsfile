pipeline {
    ...
    stages {
        ...
        stage("Docker Image Building"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                            sh "docker build -t fapi-react-app ." 
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag fapi-react-app yash5090/fapi-react-app:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/fapi-react-app:latest" 
            } 
        } 
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/fapi-react-app:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/fapi-react-app:latest'
                       sh 'docker-scout cves yash5090/fapi-react-app:latest'
                       sh 'docker-scout recommendations yash5090/fapi-react-app:latest'
                   }
                }   
            }
        }
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/fapi-react-app:latest > trivyimage.txt"   
                //}
            }
        }
